        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spu,index) in  spuSaleAttrList" :key="spu.baseSaleAttrId">
                <dt class="title">{{spu.saleAttrName}}</dt>
                <dd changepirce="20" v-for="(spus,index) in spu.spuSaleAttrValueList" :key="spus.id"
                  :class="{active:(spus.isChecked==1)}" @click="changeActive(spus,spu.spuSaleAttrValueList)">
                  {{spus.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="skuNum" @change="chageskuNum">
                <!-- 此处change为绑定函数 change为修改表单元素后触发 -->
    ...
    methods: {
      async AddShop() {
        try {
          ...
          //由于会话存储和本地存储都无法存对象所以用 JSON.stringfly转化为字符串存储  此处为会话存储
          this.$router.push({
            name: 'AddCartSuccess',
            path: '/AddCartSuccess',
